...
#define GET_SYS_TICK() erdp_if_rtos_get_system_1ms_ticks()
       ...
       public:
        UartDev() {}
        UartDev(const UartConfig_t &config, size_t recv_buffer_size) { __init(config, recv_buffer_size); }

        void init(const UartConfig_t &config, size_t recv_buffer_size) { __init(config, recv_buffer_size); }

        void send(const uint8_t *data, uint32_t len) const { erdp_if_uart_send_bytes(__uart, data, len); }

        void send(const std::vector<uint8_t> &data) const { erdp_if_uart_send_bytes(__uart, data.data(), data.size()); }
        ...
        bool recv(uint8_t &data) { return __recv_buffer.pop(data); }

        void set_usr_irq_handler(std::function<void()> usr_irq_handler) { __usr_irq_handler = usr_irq_handler; }

        void set_as_debug_com() { __debug_com = this; }

        static const UartDev *const &get_debug_com() { return __debug_com; }
       ...
       private:
        ...
        void __init(const UartConfig_t &config, size_t recv_buffer_size) {
            ...
            ERDP_UartGpioCfg_t gpio_cfg = {.tx_port = config.tx_port,
