...
OS_TaskHandle erdp_if_rtos_task_create(void (*task_function)(void *), const char *name, uint32_t stack_size, void *arg,
                                       uint32_t priority) {
...
void erdp_if_rtos_delay_ms(uint32_t nms) { vTaskDelay(pdMS_TO_TICKS(nms)); }

uint32_t erdp_if_rtos_get_1ms_timestamp(void) { return xTaskGetTickCount() * portTICK_PERIOD_MS; }

uint32_t erdp_if_rtos_get_system_heap_size(void) { return xPortGetFreeHeapSize(); }

void *erdp_if_rtos_malloc(size_t size) { return pvPortMalloc(size); }

void erdp_if_rtos_free(void *pv) { vPortFree(pv); }
...
void erdp_if_rtos_queue_delet(OS_Queue os_queue) { vQueueDelete(os_queue); }
...
uint32_t erdp_if_rtos_list_remove_item(OS_ListItem *pxItem) { return uxListRemove(pxItem); }
...
OS_Event erdp_if_rtos_event_create(void) { return xEventGroupCreate(); }
...
bool erdp_if_rtos_recursive_semaphore_give(OS_Semaphore semaphore) { return (bool)xSemaphoreGiveRecursive(semaphore); }

void erdp_if_rtos_semaphore_delet(OS_Semaphore semaphore) { vSemaphoreDelete(semaphore); }
...
static void erdp_if_rtos_timer_callback_freertos(TimerHandle_t xTimer) {
    OS_Timer timer = (OS_Timer)pvTimerGetTimerID(xTimer);
...
OS_Timer erdp_if_rtos_timer_create(const char *name, uint32_t period_ms, bool auto_reload, void (*callback)(void *),
                                   void *arg) {
    OS_Timer timer = erdp_if_rtos_malloc(sizeof(FreeRtosTimerHandler_t));
    if (timer == NULL) {
        return NULL;
    }
    ...
    timer->arg = arg;
    timer->handle =
        xTimerCreate(name, pdMS_TO_TICKS(period_ms), auto_reload, timer, erdp_if_rtos_timer_callback_freertos);
    return timer;
}
...
bool erdp_if_rtos_timer_start(OS_Timer timer) {
    ...
    if (xPortIsInsideInterrupt() == pdTRUE) {
        bool os_sta;
        BaseType_t xHigherPriorityTaskWoken = pdFALSE;
        os_sta = (bool)xTimerStartFromISR(timer->handle, &xHigherPriorityTaskWoken);
        portYIELD_FROM_ISR(xHigherPriorityTaskWoken);
        return os_sta;
    }
    return (bool)xTimerStart(timer->handle, 0);
}
...
bool erdp_if_rtos_timer_stop(OS_Timer timer) {
    ...
    if (xPortIsInsideInterrupt() == pdTRUE) {
        bool os_sta;
        BaseType_t xHigherPriorityTaskWoken = pdFALSE;
        os_sta = (bool)xTimerStopFromISR(timer->handle, &xHigherPriorityTaskWoken);
        portYIELD_FROM_ISR(xHigherPriorityTaskWoken);
        return os_sta;
    }
    return (bool)xTimerStop(timer->handle, 0);
}
...
bool erdp_if_rtos_timer_delete(OS_Timer timer) {
    ...
    erdp_if_rtos_free(timer);
    return (bool)xTimerDelete(handle, 0);
}
...
bool erdp_if_rtos_timer_reset(OS_Timer timer) {
    ...
    if (xPortIsInsideInterrupt() == pdTRUE) {
        bool os_sta;
        BaseType_t xHigherPriorityTaskWoken = pdFALSE;
        os_sta = (bool)xTimerResetFromISR(timer->handle, &xHigherPriorityTaskWoken);
        portYIELD_FROM_ISR(xHigherPriorityTaskWoken);
        return os_sta;
    }
    return (bool)xTimerReset(timer->handle, 0);
}
...
bool erdp_if_rtos_timer_is_active(OS_Timer timer) {
    ...
    return (bool)xTimerIsTimerActive(timer->handle);
}
...
bool erdp_if_rtos_timer_set_period(OS_Timer timer, uint32_t period_ms) {
    ...
    if (xPortIsInsideInterrupt() == pdTRUE) {
        bool os_sta;
        BaseType_t xHigherPriorityTaskWoken = pdFALSE;
        os_sta = (bool)xTimerChangePeriodFromISR(timer->handle, pdMS_TO_TICKS(period_ms), &xHigherPriorityTaskWoken);
        portYIELD_FROM_ISR(xHigherPriorityTaskWoken);
        return os_sta;
    }
